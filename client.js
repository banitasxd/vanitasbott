...
const connect = async () => {
   ...
   global.client = Socket({
      ...
      msgRetryCounterMap,
      browser: ['@vanitas / vanitas-bot', 'Chrome', '1.0.0'],
      auth: state,
   ...
   client.ev.on('group-participants.update', async (room) => {
      ...
      if (room.action == 'add') {
         if (groupSet.localonly) {
            if (typeof global.db.users[member] != 'undefined' && !global.db.users[member].whitelist && !member.startsWith('212') || !member.startsWith('212')) {
               client.reply(room.id, Func.texted('bold', `Sorry @${member.split`@`[0]}, this group is only for morocco people and you will removed automatically.`))
